...
def fix_annotator1(data_table):
    ...
    data_table.loc[
        data_table.Text == "HA-Fachdidaktik-Teil2_13-03-14_26-06-32_SB02", "StartChar"
    ] = (data_table["StartChar"] + 1)
    data_table.loc[
        (data_table.Text == "HA-Texthabitualisierung_15-08-26_10-19-88_SB20")
        & (data_table.StartChar > 10000),
        "StartChar",
    ] = (data_table["StartChar"] + 7)
    data_table.loc[data_table.Text == "HA-Fachdidaktik_13-03-14_26-06-32_SB02", "StartChar"] = (
        data_table["StartChar"] + 1
    )
    data_table.loc[
        data_table.Text == "Examensarbeit-Ergebnisse-Diskussion_12-06-12_27-23-27_SB15", "StartChar"
    ] = (data_table["StartChar"] + 4)
    data_table.loc[
        (data_table.Text == "Text-Beobachten_11-12-11_25-11-18_SB03")
        & (data_table.StartChar > 1000),
        "StartChar",
    ] = (data_table["StartChar"] + 1)
    ...
    data_table.loc[
        (data_table.Text == "BA-2011-Edition_15-03-09_24-24-59_SB02")
        & (data_table.StartChar > 30000),
        "StartChar",
    ] = (data_table["StartChar"] + 1)
...
def invert_filename_order(filename):
    ...
    filename_pattern = "(.*?_)([0-9]{2}-[0-9]{2}-[0-9]{2}_)(.*)"
    if re.fullmatch(filename_pattern, filename):
        filename_new = re.sub(filename_pattern, r"\2\1\3.txt", filename)
        return filename_new
    else:
...
def correct_file_names(text_name_series):
    ...
    shortend_filenames = {
        "Modulabschlusspruefung-Genderansaetze-in-der-Jugendarbeit_13-04": "Modulabschlusspruefung-Genderansaetze-in-der-Jugendarbeit_13-04-17_31-10-43",
        "HA-der-Gebrauch-juedischer-Stereotypen-in-deutschen-Medien_12-0": "HA-der-Gebrauch-juedischer-Stereotypen-in-deutschen-Medien_12-03-10_26-24-82_SB16",
        "Abstract-Zusammenhang-Einleitung-Forschungsstand_15-06-10_52-14": "Abstract-Zusammenhang-Einleitung-Forschungsstand_15-06-10_52-14-99_WB06",
        "HA-Bildungstheorie-und-qualitative-Bildungsforschung_13-03-07_2": "HA-Bildungstheorie-und-qualitative-Bildungsforschung_13-03-07_21-02-26_SB16",
        "HA-Scaffolding-Bildungssprache-Ergaenzung_13-09-16_10-07-01_SB0": "HA-Scaffolding-Bildungssprache-Ergaenzung_13-09-16_10-07-01_SB01",
    }

    filenames_mapping = {}

    for filename in text_name_series:
        filenames_mapping[filename] = invert_filename_order(
            shortend_filenames.get(filename, filename)
        )
...
def get_coder_data(annotation_directory, coders):
    ...
    for coder in coders:
        ...
        codes = dict(zip(codes["ID"], codes["Name"]))
        ...
        texts = dict(zip(texts["ID"], texts["Name"]))
        ...
        codings["index_intermediate"] = np.where(
            codings["StartChar-PDF"].str.contains(":"),
        ...
        print(
            "{} Duplikat(e) bei {} gelöscht.".format(
                items_with_duplicates - items_without_duplicates, coder
            )
        )
...
def merge_datasets(data_table):
    ...
    data_table.columns = (
        pd.Series(data_table.columns.tolist()).apply(pd.Series).sum(axis=1)
    ...
    data_table_clean = data_table_clean.rename(
        columns={
            "annotator1ID": "ID",
            "annotator1Text": "Text",
